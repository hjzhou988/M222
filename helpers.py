...
def pretty_print_aligned_reads_with_ref(genome_oriented_reads, read_alignments, ref, read_length=50,
    ...
    #only if both reads in a read pair are aligned.
    good_alignments = [len(x)==2 and (read_sep + read_length - buffer < x[1] - x[0] <
                       read_sep + read_length + buffer) for x in read_alignments]
    notthatgood_alignments=[len(x)==2 and not (read_sep + read_length - buffer < x[1] - x[0] <
                       read_sep + read_length + buffer) for x in read_alignments]
    ...
    intermediate_reads=[genome_oriented_reads[i] for i in range(len(notthatgood_alignments)) if notthatgood_alignments[i]]
    ...
    best_alignments = [read_alignments[i] for i in range(len(read_alignments))
                       if good_alignments[i]]
    intermediate_alignments=[read_alignments[i] for i in range(len(read_alignments))
                       if notthatgood_alignments[i]]
    ...
    notthatgood_aligned_reads=[intermediate_reads[i][0] + '.' * (intermediate_alignments[i][1] - intermediate_alignments[i][0] - read_length)
                     + intermediate_reads[i][1] for i in range(len(intermediate_reads))]
    aligned_reads=aligned_reads+notthatgood_aligned_reads
